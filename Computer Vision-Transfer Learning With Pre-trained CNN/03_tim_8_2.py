...
Original file is located at
    https://colab.research.google.com/drive/1ZWGpsZfUFmyQZh5huRWN-kir0b-3nV2y
...
class VisionModel(torch.nn.Module):
    def __init__(self, model_selection : str, *args, **kwargs) -> None:
        ...
        if self.model_selection == "resnet":
            ...
            self.model.classifier = torch.nn.Linear(self.model.classifier.in_features, 10) # TODO: Ubah output layer ViT dengan 10 kelas
...
EPOCH = 5
BATCH_SIZE = 64  # Set the batch size, common values are 16, 32, 64, etc.
LEARNING_RATE = 0.001  # Set the learning rate, common values are 0.1, 0.01, 0.001, etc.

"""### Define the model"""

# TODO: Isi parameter dengan "resnet" untuk ResNet18, atau "densenet" untuk DenseNet121, dan "vit" untuk Vision Transformer.
model = VisionModel("densenet").to(device)  # Ganti "resnet" sesuai model yang diinginkan
check_params(model)
...
"""Dengan menggunakan 5 iterasi pelatihan yang sama, transfer learning dengan beberapa layer yang di-freeze (dibekukan) cenderung menghasilkan akurasi akhir yang lebih rendah dibandingkan dengan model tanpa "freezing". Ketika kita membekukan layer tertentu, kita mencegahnya dari proses pembaruan parameter, sehingga layer-layer tersebut hanya menggunakan pengetahuan yang sudah diperoleh dari data sebelumnya tanpa benar-benar beradaptasi dengan pola spesifik pada data baru, seperti dataset MNIST. Meskipun akurasi validasi dapat meningkat dengan cepat pada tahap awal, model akan mencapai batas karena terbatasnya fleksibilitas untuk mengoptimalkan parameter pada layer yang di-freeze. Akibatnya, model yang dibekukan memiliki kapasitas belajar yang lebih terbatas, sehingga kurang mampu mengidentifikasi fitur spesifik pada dataset target. Sebaliknya, model tanpa layer yang di-freeze mampu menyesuaikan parameter secara penuh, memungkinkan model untuk menangkap pola dan karakteristik baru dalam data target dengan lebih baik, yang berkontribusi pada akurasi akhir yang lebih tinggi."""

# [ PERTANYAAN ]
# TODO: Mengapa semakin banyak layer yang di-freeze, semakin rendah akurasi model di awal iterasi (epoch)?

"""Ketika semakin banyak layer pada model yang di-freeze (dibekukan), akurasi awal model pada iterasi pertama (epoch) cenderung lebih rendah. Hal ini disebabkan oleh keterbatasan model dalam menyesuaikan parameter pada layer-layer yang dibekukan tersebut, yang sudah terlatih pada dataset yang berbeda (misalnya, ImageNet). Karena parameter pada layer yang dibekukan tidak lagi dapat dioptimalkan, model kehilangan fleksibilitas dalam menangkap karakteristik unik dari dataset baru, seperti MNIST, yang memiliki pola data yang berbeda.

Layer yang di-freeze tidak beradaptasi dengan pola data dari dataset target sehingga model hanya dapat memperbarui parameter di layer atas (yang tidak dibekukan) untuk menyesuaikan diri dengan dataset baru. Dengan lebih banyak layer yang dibekukan, ruang parameter yang dapat dioptimalkan menjadi lebih kecil, membatasi kemampuan model untuk mempelajari fitur-fitur unik dari dataset target. Pada awal iterasi, model masih sangat bergantung pada lapisan atas yang relatif dangkal dan belum memanfaatkan fitur dasar yang spesifik untuk dataset target. Akibatnya, akurasi model lebih rendah pada awal iterasi dan proses adaptasi menjadi lebih lambat.

Seiring berjalannya proses training, model mungkin mampu meningkatkan akurasinya dengan lebih banyak iterasi. Namun, karena keterbatasan dalam ruang parameter yang bisa diperbarui, model membutuhkan lebih banyak waktu untuk belajar dan menyesuaikan diri dengan pola data pada dataset target, seperti MNIST.
...
"""Semakin banyak layer yang di-freeze, semakin sedikit parameter yang perlu diperbarui selama proses training. Hanya layer yang tidak di-freeze yang dihitung gradiennya dan di-update bobotnya, sehingga proses komputasi per iterasi menjadi lebih ringan. Dengan berkurangnya jumlah operasi matematika yang diperlukan, forward dan backward pass menjadi lebih cepat, yang secara langsung mengurangi waktu training dan validasi. Layer yang di-freeze tidak mengalami perubahan pada bobot dan biasnya, sehingga proses backpropagation hanya dilakukan pada layer-layer yang tidak di-freeze. Dengan kata lain, semakin sedikit parameter yang di-update, semakin cepat pula waktu training dan validasinya.

### Scoring
Total `#TODO` = 12
<br>Checklist:

- [x] Ubah susunan input layer DenseNet dengan memanggil create_conv2d()
- [x] Ubah output layer DenseNet dengan 10 kelas
- [x] Ubah susunan input layer ViT dengan memanggil create_conv2d()
- [x] Ubah output layer ViT dengan 10 kelas
- [x] Tentukan nilai batch size
- [x] Tentukan nilai learning rate
- [x] Tentukan loss function (untuk multi-classification)
- [x] Isi parameter dengan "resnet" untuk ResNet18, atau "densenet" untuk DenseNet121, dan "vit" untuk Vision Transformer.
- [x] Panggil model, lalu tentukan besaran epoch, train data loader, dan validation data loader
- [x] PERTANYAAN: Dengan 5 iterasi yang sama pada training, mengapa Transfer Learning dengan layer yang di-freeze lebih buruk di akurasi akhir?
- [x] PERTANYAAN: Mengapa semakin banyak layer yang di-freeze, semakin rendah akurasi model di awal iterasi (epoch)?
- [x] PERTANYAAN: Mengapa semakin banyak layer yang di-freeze, semakin cepat waktu training dan validasinya?
